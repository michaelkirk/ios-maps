...
struct FrontPageSearch: View {
  ...
  @StateObject var preferences = PreferencesController.shared.preferences
        ...
        if let places = places {
          ...
          .onChange(of: selectedPlace) { newValue in
            if newValue != nil {
              ...
              PreferencesController.shared.addSearch(text: queryText)
            }
          }
        } else {
          if !preferences.recentSearches.isEmpty {
            VStack(alignment: .leading, spacing: 8) {
              HStack {
                ...
                Spacer()
                Button(action: {
                  PreferencesController.shared.clear()
                }) {
                  Text("Clear")
                }
              }
